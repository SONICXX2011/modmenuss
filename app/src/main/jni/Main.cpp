...
#define OFFSET_SET_TEXT   0x205DA44     // InputField.set_text (از دامپ)
...
static void inject_ip_to_game(JNIEnv* env, jobject obj) {
    ...
    try {
        ...
        write_debug("🔄 Calling set_text with jstring");
        ...
        if (env->ExceptionCheck()) {
            ...
            env->ExceptionClear();
            write_debug("⚠️ Exception during set_text call!");
            write_log(g_ipResultLog, "⚠️ Exception during set_text!");
        } else {
            write_debug("✅ set_text called successfully: " + g_targetIP);
